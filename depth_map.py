...
def crop_face(mat, kpt, scale, ori_shape, resize_shape):
    ...
    crop_face_mat = cv2.resize(crop_face_mat, tuple(
        resize_shape), interpolation=cv2.INTER_CUBIC)
...
def convert_vertices_2_map_interp2d_directly(vertices, shape):
    ...
    x1, x2, y1, y2 = np.min(vertices[:, 0]), np.max(
        vertices[:, 0]), np.min(vertices[:, 1]), np.max(vertices[:, 1])
    xnew, ynew = np.meshgrid(np.linspace(
        x1, x2, int(x2-x1)), np.linspace(y1, y2, int(y2-y1)))
    #ynew = ynew.flatten()
    #xnew = xnew.flatten()

    znew = interpolate.griddata(
        points, z, (xnew, ynew), method='linear', fill_value=0)
    znew[znew == np.nan] = 0
